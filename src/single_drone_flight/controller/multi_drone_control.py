...
class DroneSquareControl(Node):
    """
    Square-flight mission for PX4 Offboard, namespaced for multi-drone.
    """

    def __init__(self, namespace="drone1"):
        super().__init__(f"{namespace}_square_control")

        self.ns = f"/{namespace}"  # ROS 2 namespace for this drone
        ...
        self.attitude_sub = self.create_subscription(
            VehicleAttitude, f"{self.ns}/fmu/out/vehicle_attitude", self.attitude_callback, qos_profile
        )
        self.local_position_sub = self.create_subscription(
            VehicleLocalPosition, f"{self.ns}/fmu/out/vehicle_local_position", self.local_position_callback, qos_profile
        )
        ...
        # --- Timers ---
        self.timer_hz = 50.0
        self.timer = self.create_timer(1.0 / self.timer_hz, self.control_loop_callback)  # 50 Hz
        ...
        self.offboard_warmup_duration = 1.0  # seconds of pre-stream before switching
        ...
        self.get_logger().info(f"[{namespace}] Drone Square Control Node Initialized")
    ...
    def local_position_callback(self, msg: VehicleLocalPosition):
        ...
        self.current_position.z = msg.z  # Down (negative up)
    ...
    def set_land_mode(self):
        # Correct: VEHICLE_CMD_NAV_LAND (id 21), not 400
    ...
    def create_square_waypoints(self, side=4.0):
        ...
        self.square_points = [
            ...
            (cx - half, cy - half),  # WP2
            (cx + half, cy - half),  # WP3
            (cx + 0.0, cy + 0.0),    # WP4 back to center
        ]
    ...
    def state_machine_callback(self):
        ...
        if self.landing_initiated:
            # Transition to LANDED when disarmed or after AUTO_LAND finishes
        ...
        if self.current_state == "IDLE":
            ...
            # Send LAND once, then mark landing_initiated and let PX4 finish
            if not self.landing_cmd_sent:
                self.set_land_mode()  # sends VEHICLE_CMD_NAV_LAND
                self.landing_cmd_sent = True
                self.landing_initiated = True
                self.get_logger().info("LAND command sent; handing control to PX4")
            # We do NOT send position/velocity setpoints here.
    ...
    def control_loop_callback(self):
        ...
        if self.current_state == "REACHING_TARGET":
            # After Offboard is ON, use a small vertical velocity until close to target altitude
            if self.offboard_mode:
                offboard_msg.velocity = True
                offboard_msg.position = False
                traj.velocity[2] = self.climbing_velocity if (self.current_altitude - self.target_altitude) > 0.2 else 0.0
                traj.yaw = 0.0
        ...
        # If no control bit set (e.g., only warmup path), default to position @ target altitude
...
def main(args=None):
    ...
    # --- Example: launch controllers for multiple drones ---
    # You can set this list dynamically from a launch file
    drone_namespaces = ["drone1", "drone2", "drone3"]

    nodes = [DroneSquareControl(ns) for ns in drone_namespaces]
